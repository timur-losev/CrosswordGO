...
public class Pan : MonoBehaviour
{
    ...
    private List<char> uniqueLetters = new List<char>();
    ...
    public void Load(GameLevel gameLevel)
    {
        ...
        BuildLettersFromCrossword();
        numLetters = uniqueLetters.Count;
        ...
        for(int i = 0; i < numLetters; i++)
        {
            ...
            letter.text = uniqueLetters[i].ToString().ToUpper();
    ...
    private void GetPanWord()
    {
        ...
        for (int i = 0; i < numLetters; i++)
        {
            sb.Append(uniqueLetters[indexes[i]]);
        }
        panWord = sb.ToString();
    ...
    private void BuildLettersFromCrossword()
    {
        uniqueLetters.Clear();
        string path = WordRegion.GetCrosswordConfigPath(world, subWorld, level);
        var crosswordConfigs = CrosswordLoader.LoadCrosswordConfig(path);
        if (crosswordConfigs != null && crosswordConfigs.Count > 0)
        {
            HashSet<char> seen = new HashSet<char>();
            foreach (var cfg in crosswordConfigs)
            {
                string answer = cfg.Answer.ToUpper();
        ...
        // Fallback to original word letters if config отсутствует
        if (uniqueLetters.Count == 0)
        {
            foreach (char c in gameLevel.word.ToUpper())
            {
